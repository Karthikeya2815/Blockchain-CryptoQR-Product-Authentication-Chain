...
contract product {
    ...
    mapping(bytes32 => Scan[]) public productScans;
    ...
    uint256 _productPrice, string memory _mfgLocation) public{
        productItems[productCount] = productItem(productCount, _productSN, _productName, _productBrand,
        _productPrice, "Available", block.timestamp, 0, false, _mfgLocation, "");
    ...
    function sellerSellProduct(bytes32 _productSN, bytes32 _consumerCode, string memory _purchaseLocation) public{   
        uint256 j = productMap[_productSN];

        if(productItems[j].productSN == _productSN && productItems[j].productStatus == "Available") {
            productItems[j].productStatus = "NA";
            productItems[j].saleDate = block.timestamp;
            productItems[j].purchaseLocation = _purchaseLocation;
    ...
    //Verify and Record Scan

    function recordScan(bytes32 _productSN, string memory _location) public {
        uint256 j = productMap[_productSN];
        
        // Only record and check logic if product actually exists
        if (productItems[j].productSN == _productSN) {
            productScans[_productSN].push(Scan(block.timestamp, _location));

            // Logic 1: Suspicious if scanned after sale date
            if (productItems[j].saleDate > 0 && block.timestamp > productItems[j].saleDate + 60) {
                productItems[j].isSuspicious = true;
            }

            // Logic 2: Suspicious if simultaneous scans in different locations
            uint256 scanCount = productScans[_productSN].length;
            if (scanCount > 1) {
                Scan memory lastScan = productScans[_productSN][scanCount - 2];
                if (block.timestamp - lastScan.timestamp < 1 hours) {
                    if (keccak256(abi.encodePacked(lastScan.location)) != keccak256(abi.encodePacked(_location))) {
                        productItems[j].isSuspicious = true;
                    }
                }
            }
    ...
    function getScanHistory(bytes32 _productSN) public view returns (uint256[] memory, string[] memory) {
        uint256 count = productScans[_productSN].length;
        uint256[] memory timestamps = new uint256[](count);
        string[] memory locations = new string[](count);

        for (uint i = 0; i < count; i++) {
            timestamps[i] = productScans[_productSN][i].timestamp;
            locations[i] = productScans[_productSN][i].location;
        }
        return (timestamps, locations);
    }

    function verifyProduct(bytes32 _productSN, bytes32 _consumerCode) public view returns(bool, bool, uint256, bool, bool, string memory, string memory, uint256, uint256){
        uint256 j = productMap[_productSN];
        
        return (
            (productItems[j].productSN == _productSN),
            (productsSold[_productSN] == _consumerCode),
            productScans[_productSN].length,
            productItems[j].isSuspicious,
            (productItems[j].saleDate > 0),
            productItems[j].mfgLocation,
            productItems[j].purchaseLocation,
            productItems[j].mfgDate,
            productItems[j].saleDate
        );
    }
}